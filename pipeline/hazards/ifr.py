...
draws by hand in N-AWIPS -- lots of tiny, separate, jagged-edged
polygons that wouldn't look like an operational product. Three
processing steps close that gap, applied in this order:

  1. Neighborhood-maximum smoothing (pipeline.smoothing) -- pulls
     nearby smaller hazard areas into larger ones, the way a forecaster
     wouldn't draw two separate tiny blobs 20 miles apart if they're
     part of the same weather feature.
  2. Gaussian smoothing -- knocks down small-scale grid noise before
     contouring, so boundaries come out smoother rather than
     pixel-jagged.
  3. Geodesic-area filtering + boundary smoothing (pipeline.polygons) --
     drops anything smaller than the real AIRMET/G-AIRMET "widespread"
     criterion (historically 3,000 sq mi), and rounds off remaining
     jagged edges into something closer to hand-drawn.
"""
...
# Fixed (not forecaster-exposed) cosmetic smoothing parameters -- these
# affect HOW smooth things look, not what counts as a hazard, so unlike
# threshold/neighborhood-radius/min-area they're not exposed as
# forecaster-adjustable knobs. Easy to promote to parameters later if
# that turns out to matter.
GAUSSIAN_SIGMA_CELLS = 1.5
BOUNDARY_SMOOTHING_DEG = 0.05
...
def generate_ifr_polygons(
    ...
    smooths them, and returns a GeoJSON FeatureCollection of IFR hazard
    polygons shaped to resemble a forecaster-drawn product rather than
    raw NBM-resolution detail.
    ...
    neighborhood_radius_nm : float
        Real-world radius (nautical miles) used to pull nearby smaller
        hazard areas into larger ones (see pipeline.smoothing). 0
        disables this. Forecaster-adjustable.
    min_area_sq_mi : float
        Polygons smaller than this (true geodesic area) are dropped.
        Matches AIRMET/G-AIRMET's historical 3,000 sq mi "widespread"
        criterion by default. Forecaster-adjustable.
    ...
    # just outside the native grid's convex hull, and both the
    # neighborhood-max and Gaussian filters below would otherwise
    # spread that NaN into a much larger surrounding area than the
    # original gap (maximum_filter/gaussian_filter both propagate NaN
    # to any output cell whose input footprint touches one).
    combined = np.maximum(np.nan_to_num(ceil_regridded), np.nan_to_num(vis_regridded))

    combined = neighborhood_max_smooth(combined, grid_spec, radius_nm=neighborhood_radius_nm)
    ...
    polygons = grid_to_polygons(
        ...
        boundary_smoothing_deg=BOUNDARY_SMOOTHING_DEG,
    )
